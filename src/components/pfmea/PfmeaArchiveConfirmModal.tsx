...
interface PfmeaArchiveConfirmModalProps {
  pfmea: PfmeaHeader;
  onConfirm: () => Promise<void>;
  onCancel: () => void;
}

export function PfmeaArchiveConfirmModal({
  pfmea,
  onConfirm,
  onCancel,
}: PfmeaArchiveConfirmModalProps) {
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-amber-900 dark:text-amber-100">
                {t('archive.modal.title', 'Archivar Documento')}
              </h3>
              <p className="text-xs text-steel-500 dark:text-steel-400 mt-0.5">{t('archive.modal.subtitle', 'Esta acción es permanente para esta versión')}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-5">
            ...
            <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/25 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
                  {t('archive.modal.warningTitle', 'Advertencia')}
                </p>
                <p className="text-xs text-amber-700 dark:text-amber-400/80 mt-1 leading-relaxed">
                  {t('archive.modal.warningDesc', 'Al archivar este documento, dejará de estar disponible para edición y pasará a solo lectura.')}
                </p>
              </div>
            </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/25 rounded-xl p-4 flex flex-col gap-3">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="text-sm font-semibold text-amber-900 dark:text-amber-200 line-clamp-1" title={pfmea.project_name}>
                    {pfmea.project_name}
                  </h4>
                  <div className="flex items-center gap-2 mt-1 text-xs text-amber-700/80 dark:text-amber-400/80">
                    <span className="font-mono text-amber-800 dark:text-amber-300">#{pfmea.id}</span>
                    <span className="text-amber-400 dark:text-amber-600/50">•</span>
                    <span className="font-mono text-amber-800 dark:text-amber-300">v{pfmea.version}</span>
                    <span className="text-amber-400 dark:text-amber-600/50">•</span>
                    <span>{new Date(pfmea.created_at).toLocaleDateString(undefined, { dateStyle: 'short' })}</span>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-amber-100 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-[10px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                  ...
                  {t(`status.${(pfmea.status || 'Draft').toLowerCase()}`, pfmea.status || 'Draft')}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-steel-500 dark:text-steel-400">
                ...
                {t('archive.modal.reasonLabel', 'Motivo')}
                <span className="text-red-400">*</span>
              </label>
              <textarea
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder={t('archive.modal.reasonPlaceholder', 'Describe el motivo por el cual archivas el documento...')}
                rows={3}
              ...
              <div className="flex justify-between items-center">
                {reason.trim().length > 0 && reason.trim().length < 10 && (
                  <p className="text-xs text-red-500 dark:text-red-400">{t('archive.errors.reasonRequired', 'El motivo debe tener al menos 10 caracteres')}</p>
                )}
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <p className="text-xs text-amber-800/80 dark:text-amber-400/80 leading-relaxed group-hover:text-amber-900 dark:group-hover:text-amber-300 transition-colors">
                {t('archive.modal.confirmCheck', 'Entiendo que esta acción es permanente')}
              </p>
            </label>
          </div>
          ...
          <div className="px-6 pb-6 flex items-center justify-end gap-3 border-t border-amber-200 dark:border-amber-500/20 pt-4">
            ...
            >
              {t('archive.modal.cancelBtn', 'Cancelar')}
            </button>
            <motion.button
              type="submit"
            ...
            >
              ...
              {t('archive.modal.archiveBtn', 'Archivar')}
            </motion.button>
          </div>
        </form>
      </motion.div>
